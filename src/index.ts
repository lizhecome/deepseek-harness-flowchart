...
function parseArgs(args: RenderArgs, config: ResolvedConfig): ParsedArgs {
  const source = args.source.trim()
  if (source.length === 0) throw new Error('source must not be blank')
  ...
  if (filePath.length === 0) throw new Error('file_path must not be blank')
  ...
  if (colors === undefined) throw new Error(`unknown theme ${JSON.stringify(theme)}`)
  return {
    source,
    filePath,
    theme,
...
function prepareSvg(svg: string, maxBytes: number): { svg: string; bytes: number } {
...
function nestedWriteError(result: Extract<Awaited<ReturnType<Context['tools']['execute']>>, { isError: true }>): Error {
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (writeResult.isError) throw nestedWriteError(writeResult)
